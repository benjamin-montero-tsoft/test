 ...
 * Noncommercial-Share Alike 3.0 Unported License except in jurisdicitons for
 * which the license has been ported by Creative Commons International, where
 * the work is licensed under the applicable ported license instead. For a copy
 * of the unported license, visit
 * http://creativecommons.org/licenses/by-nc-sa/3.0/ or send a letter to
 * Creative Commons, 171 Second Street, Suite 300, San Francisco, California,
 * 94105, USA. For ported versions of the license, visit
 * http://creativecommons.org/international/
 * 
 * Alternative licensing arrangements may be made by contacting the author at
 * http://www.AMA3.com/contact/
 * 
 * The Any+Time(TM) JavaScript Library provides the following ECMAScript
 * functionality:
 * 
 * AnyTime.Converter Converts Dates to/from Strings, allowing a wide range of
 * formats closely matching those provided by the MySQL DATE_FORMAT() function,
 * with some noteworthy enhancements.
 * 
 * AnyTime.pad() Pads a value with a specific number of leading zeroes.
 * 
 * AnyTime.noPicker() Destroys a calendar widget previously added by
 * AnyTime.picker(). Can also be invoked via jQuery using
 * $(selector).AnyTime_noPicker()
 * 
 * AnyTime.picker() Attaches a calendar widget to a text field for selecting
 * date/time values with fewer mouse movements than most similar pickers. Any
 * format supported by AnyTime.Converter can be used for the text field. If
 * JavaScript is disabled, the text field remains editable without any of the
 * picker features. Can also be invoked via jQuery using
 * $(selector).AnyTime_picker()
 * 
 ...
 * 10.10 and Safari 4.0. Minor variations in IE6+7 are to be expected, due to
 * their broken box model. Please report any other problems to the author (URL
 * above).
 * 
 * Any+Time is a trademark of Andrew M. Andrews III. Thanks to Chu for help with
 * a setMonth() issue!
 ******************************************************************************/
...
(function($) {
	...
	jQuery.prototype.AnyTime_height = function(inclusive) {
		return (__msie6 ? Number(this.css('height').replace(/[^0-9]/g, ''))
				: this.outerHeight(inclusive));
	};

	jQuery.prototype.AnyTime_width = function(inclusive) {
		return (__msie6 ? (1 + Number(this.css('width').replace(/[^0-9]/g, '')))
	...
	//
	// options - an object of optional parameters that override default
	// behaviors.
	...
	AnyTime.Converter = function(options) {
		...
		this.fmt = '%Y-%m-%d %T';
		this.dAbbr = [ 'Dom', 'Lun', 'Mar', 'Mie', 'Jue', 'Vie', 'Sab' ];
		this.dNames = [ 'Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday',
				'Friday', 'Saturday' ];
		this.eAbbr = [ 'BCE', 'CE' ];
		this.mAbbr = [ 'Ene', 'Feb', 'Mar', 'Abr', 'May', 'Jun', 'Jul', 'Ago',
				'Sep', 'Oct', 'Nov', 'Dic' ];
			...
			for ( var f = 0; f < _flen; f++) {
				if (this.fmt.charAt(f) != '%')
					...
					case 'a': // Abbreviated weekday name (Sun..Sat)
						...
						break;
					case 'B': // BCE string (eAbbr[0], usually BCE or BC, only
								// if appropriate) (NON-MYSQL)
						if (d.getFullYear() < 0)
							str += this.eAbbr[0];
						break;
					case 'b': // Abbreviated month name (Jan..Dec)
						...
						break;
					case 'C': // CE string (eAbbr[1], usually CE or AD, only
								// if appropriate) (NON-MYSQL)
						if (d.getFullYear() > 0)
							str += this.eAbbr[1];
						break;
					...
					case 'd': // Day of the month, numeric (00..31)
						...
						str += String(t);
						break;
					case 'D': // Day of the month with English suffix (0th,
								// 1st,...)
						t = String(d.getDate());
						str += t;
						...
						break;
					case 'E': // era string (from eAbbr[], BCE, CE, BC or AD)
								// (NON-MYSQL)
					...
					case 'w': // Day of the week (0=Sunday..6=Saturday)
						str += d.getDay();
						break;
					case 'Y': // Year, numeric, four digits (negative if
								// before 0001)
						str += AnyTime.pad(d.getFullYear(), 4);
						break;
					case 'y': // Year, numeric (two digits, negative if before
								// 0001)
						t = d.getFullYear() % 100;
						str += AnyTime.pad(t, 2);
						break;
					case 'Z': // Year, numeric, four digits, unsigned
								// (NON-MYSQL)
						str += AnyTime.pad(Math.abs(d.getFullYear()), 4);
						break;
					case 'z': // Year, numeric, variable length, unsigned
								// (NON-MYSQL)
					...
					case '+': // signed, 4-digit timezone offset in hours and
								// minutes
					case '-': // signed, 3-or-4-digit timezone offset in hours
								// and minutes
					case ':': // signed 4-digit timezone offset with colon
								// delimiter
					case ';': // signed 3-or-4-digit timezone offset with
								// colon delimiter
						t = (_offAl != Number.MIN_VALUE) ? _offAl
								: (_offF == Number.MIN_VALUE) ? (0 - d
										.getTimezoneOffset()) : _offF;
					...
					case 'U': // Week (00..53), where Sunday is the first day
								// of the week
					case 'u': // Week (00..53), where Monday is the first day
								// of the week
					case 'V': // Week (01..53), where Sunday is the first day
								// of the week; used with %X
					case 'v': // Week (01..53), where Monday is the first day
								// of the week; used with %x
					case 'X': // Year for the week where Sunday is the first
								// day of the week, numeric, four digits; used
								// with %V
					case 'x': // Year for the week, where Monday is the first
								// day of the week, numeric, four digits; used
								// with %v
						throw '%' + ch
		...
		// last captured by a parsed string (or assumed by
			...
			for ( var f = 0; f < _flen; f++) {
				if (this.fmt.charAt(f) == '%') {
					...
					case 'a': // Abbreviated weekday name (Sun..Sat)
						...
						break;
					case 'B': // BCE string (eAbbr[0]), only if needed.
								// (NON-MYSQL)
					...
					case 'b': // Abbreviated month name (Jan..Dec)
						...
						break;
					case 'C': // CE string (eAbbr[1]), only if needed.
								// (NON-MYSQL)
					...
					case 'Y': // Year, numeric, four digits, negative if
						...
						s += i;
						break;
					case 'y': // Year, numeric (two digits), negative before
								// baseYear
						i = 2;
						if (str.substr(s, 1) == '-')
							i++;
					...
					case '@': // timezone label
						...
						if (AnyTime.utcLabel) {
							...
							for (tzOff in AnyTime.utcLabel)
								if (!Array.prototype[tzOff]) // prototype.js
																// compatibility
																// issue
								{
	...
	// </script>
	//
	// The appearance of the picker can be extensively modified using CSS
	// styles.
	...
	AnyTime.picker = function(id, options) {
		...
		__pickers[id] = {
			...
			initialize : function(id) {
				...
				this.inp = $(document.getElementById(id)); // avoids
															// ID-vs-pseudo-selector
															// probs like
															// id="foo:bar"
				this.div = $('<div class="AnyTime-win AnyTime-pkr ui-widget ui-widget-content ui-corner-all" style="width:0;height:0" id="'
				...
				if (askDate) {
					...
					if (askDoM) {
						lab = options.labelDayOfMonth || 'Día del Mes';
				...
				if (askTime) {
					...
					if (askMinute) {
						...
						lab = options.labelMinute || 'Minutos';
						this.dM
					...
					if (askSec) {
						...
						lab = options.labelSecond || 'Segundos';
						this.dS
					...
					if (askOff) {
						...
						this.oCur = this.btn(offDiv, '', this.newOffset, [
								'off', 'off-cur' ], lab);
						...
						this.oSel = this.btn(offDiv, '&#177;', this.newOffset,
				...
				// Otherwise, determine a worthy title based on which (and how
				// many)
				...
				if (options.labelTitle)
					...
					this.hTitle.append('Seleccionar '
							+ (askDate ? (askTime ? 'Fecha y Hora' : 'Fecha')
									: 'Tiempo'));
				else
					this.hTitle.append('Seleccionar');
				...
				// the picker. Add the picker to the instances list (which is
				// used
			...
			askOffset : function(event) {
				if (!this.oDiv) {
					...
					// the order here (HDR,BODY,XDIV,TITLE) is important for
					// width calcluation:
					...
					var oBW = title.AnyTime_width(true) - oBWS; // initial body
																// width
					...
					this.oBody.find('.AnyTime-off-off-btn').width(btnW); // set
																			// uniform
																			// button
																			// width
					if (last) {
					...
					if (oH > oHmax) {
						oH = oHmax;
						this.oBody.height(oH
								- (title.AnyTime_height(true) + oBHS));
						this.oBody.width(this.oBody.width() + 20); // add
																	// nominal
																	// px for
																	// scrollbar
					...
					if (btnT - btnH < oBT) // move a page up
						this.oBody
								.scrollTop(btnT + this.oBody.scrollTop()
										...
										+ (btnH * 2));
					else if (btnT + btnH > oBT + this.oBody.innerHeight()) // move
																			// a
																			// page
																			// down
			...
			// appropriate action is performed according to the WAI-ARIA
			// Authoring
			...
			//
			// The authoring practices do not specify behavior for a time
			// picker,
			// or for month-and-year pickers that do not have a
			// day-of-the-month,
			// but AnyTime.picker uses the following behavior to be as
			// consistent
			...
			//
			// Because AnyTime.picker sets a value as soon as the button
			// receives
			// focus, SPACE and ENTER are not needed (the WAI-ARIA guidelines
			// use
			...
			key : function(event) {
				...
				if (key == 16) // Shift
				{
				} else if ((key == 10) || (key == 13) || (key == 27)) // Enter
																		// & Esc
				{
					if (this.oDiv && this.oDiv.is(':visible'))
						...
						this.dismiss(event);
				} else if ((key == 33) || ((key == 9) && event.shiftKey)) // PageUp
																			// &
																			// Shift+Tab
				{
				...
				} else if (key == 37) // left arrow
				{
					if (this.fBtn.hasClass('AnyTime-dom-btn'))
						this.keyDateChange(new Date(this.time.getTime()
								- __oneDay));
					else
						this.keyBack();
			...
			// ---------------------------------------------------------------------
			// .newOffset() is called when a user clicks the UTC offset
			// (timezone)
			// (or +/- button) to shift the year. It changes the date and
			// updates
			// the text field.
			// ---------------------------------------------------------------------
			...
			pos : function(event) // note: event is ignored but this is a
									// handler
			{
				if (this.pop) {
			...
			// element. The current value in the input field is used to
			// initialize
			// the picker.
			// ---------------------------------------------------------------------
				...
				if (this.pop && __iframe)
					setTimeout(
							function() {
								...
								__iframe
										.css({
											height : String(_this.div
													.outerHeight(true))
													+ 'px',
											left : String(pos.left) + 'px',
											position : 'absolute',
											top : String(pos.top) + 'px',
											width : String(_this.div
													.outerWidth(true))
													+ 'px'
										});
								__iframe.show();
			...
			upd : function(fBtn) {
				...
				if (this.dY) {
					...
					totH = this.yLab.AnyTime_height(true)
				...
				if (this.dMo) {
					...
					if (dMoW > totW)
						totW = dMoW;
					totH += this.hMo.AnyTime_height(true)
				...
				if (this.dDoM) {
					...
					totH += this.hDoM.AnyTime_height(true)
							+ this.dDoM.AnyTime_height(true);
				}
				if (this.dD) {
					this.dD.css({
						width : String(totW) + 'px',
						height : String(totH) + 'px'
					});
					totW += this.dMinW;
					totH += this.dMinH;
				}
				...
				if (this.dT) {
					this.dT.css({
						width : String(timeW) + 'px',
						height : String(timeH) + 'px'
					});
					timeW += this.tMinW + 1;
					timeH += this.tMinH;
					totW += timeW;
					...
					if (this.dO) // stretch offset button if possible
					{
						...
						this.oCur.css({
							width : String(dOW - (this.oListMinW + 4)) + "px"
						});
					}
				}
			...
			// called after most events to make the picker reflect the
			// currently-
